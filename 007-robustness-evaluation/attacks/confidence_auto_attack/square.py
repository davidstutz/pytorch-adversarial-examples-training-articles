...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        y_corr = probs[u, y].clone()
        probs[u, y] = -float('inf')
        y_others = probs.max(dim=-1)[0]

        if not self.targeted:
            if self.loss == 'conf':
                return y_corr - y_others, y_others
            elif self.loss == 'conf-margin':
    ...
    def init_hyperparam(self, x):
        ...
        assert self.loss in ['conf', 'conf-margin']
        ...
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                for i_iter in range(self.n_queries):
                    idx_to_fool = (margin_min > 0.0).nonzero().squeeze()
                    if self.best_loss:
                        idx_to_fool = torch.ones_like(idx_to_fool)
                    ...
                    if not self.best_loss and (ind_succ.numel() == n_ex_total):
                        break
