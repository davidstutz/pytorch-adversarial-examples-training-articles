...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
                 attacks_to_run=[], version='standard-conf', is_tf_model=False,
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, n_restarts=5, n_iter=1000, verbose=self.verbose,
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=1000, verbose=self.verbose,
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False):
        ...
        n_cls = check_range_output(self.get_logits, x_orig[:bs].to(self.device),
            logger=self.logger)
        check_dynamic(self.model, x_orig[:bs].to(self.device), self.is_tf_model,
            logger=self.logger)
        check_n_classes(n_cls, self.attacks_to_run, self.apgd_targeted.n_target_classes, logger=self.logger)
        ...
        with torch.no_grad():
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    elif attack == 'apgd-conf-t':
                        # targeted apgd
                        self.apgd_targeted.loss = 'conf-targeted'
    ...
    def set_version(self, version='standard-conf'):
        if self.verbose:
            print('setting parameters for {} version'.format(version))
        
        if version == 'standard-conf':
            self.attacks_to_run = ['apgd-conf', 'apgd-conf-t', 'square']
            if self.norm in ['Linf', 'L2']:
                self.apgd.n_restarts = 10
