...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if not self.is_tf_model:
            ...
            elif self.loss == 'conf-targeted':
                criterion_indiv = self.conf_loss_targeted
        ...
        #alpha = 2. if self.norm in ['Linf', 'L2'] else 1. if self.norm in ['L1'] else 2e-
        alpha = 0.05 if self.norm in ['Linf', 'L2'] else 1. if self.norm in ['L1'] else 2e-2
        ...
        counter3 = 0
        print('step size', step_size[0].item())
        ...
        for i in range(self.n_iter):
            ...
            if self.verbose:
                print('[m] iteration: {} - mean loss: {:.6f} - step size: {:5f} - robust accuracy: {:.2%}'.format(
                    i, loss_best.mean(), step_size.mean(), acc.float().mean()))
...
class APGDAttack_targeted(APGDAttack):
    ...
    def conf_loss_targeted(self, x, y):
        x = torch.nn.functional.softmax(x, dim=1)
        return x[torch.arange(x.shape[0]), self.y_target]
    ...
    def perturb(self, x, y=None, x_init=None):
        ...
        assert self.loss in ['dlr-targeted', 'conf-targeted'] #'ce-targeted'
        if not y is None and len(y.shape) == 0:
            x.unsqueeze_(0)
            y.unsqueeze_(0)
        self.init_hyperparam(x)
